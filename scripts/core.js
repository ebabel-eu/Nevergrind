...
function loadAllCharacters() {
    ...
    if (location.protocol === "http:") {
        for (var i = 1; i <= 16; i++) {
            characterslot = i;
            if (localStorage.getItem("my" + (lsKey + characterslot)) !== null) {
                var foo1 = localStorage.getItem("my" + (lsKey + characterslot));
                my = JSON.parse(foo1);
                if (i === GLB.lastCharacter && loadLastCharacter === false) {
                    loadLastCharacter = true;
                }
				var qux = localStorage.getItem("eq" + (lsKey + characterslot));
				P.eq = JSON.parse(qux);
            }else{
				my.name = '';
			}
			...
			if (localStorage.getItem("inv" + (lsKey + characterslot + 10)) === null) {
				initializeInventory();
			} else {
				var bar = localStorage.getItem("inv" + (lsKey + characterslot + 10));
				P.item = JSON.parse(bar);
			}
			if (localStorage.getItem("Q" + (lsKey + characterslot)) === null) {
				initQ();
			} else {
				var bar = localStorage.getItem("Q" + (lsKey + characterslot));
				P.Q = JSON.parse(bar);
			}
            if (my.name == "") {
                D.getElementById('characterslot' + i).style.display = 'none';
                if (firstEmptyCharacterSlot === 0) {
                    firstEmptyCharacterSlot = i;
                }
            } else {
                D.getElementById('characterslot' + i).innerHTML =
                    '<div class="yellow glowYellow nomouse">' + my.name + '</div>' +
                    '<div class="nomouse">Level ' + my.level + '</div>' +
                    '<div class="nomouse">' + my.race + ' ' + my.job + '</div>';
                D.getElementById('characterslot' + i).style.display = 'inline-block';
                charactersFound++;
        ...
        if (foundCharacter === false) {
            D.getElementById('characterSelectScreen').style.display = 'block';
            D.getElementById('createWindowId').style.display = 'block';
            D.getElementById('deletecharacter').style.display = 'none';
            D.getElementById('characterSelectScreen').style.opacity = 1;
        } else {
            if (GLB.lastCharacter !== undefined &&
                loadLastCharacter === true &&
                $("#characterslot" + GLB.lastCharacter).length > 0) {
                loadCharacterSlot(GLB.lastCharacter);
            } else {
                loadCharacterSlot(goToSlot);
            }
        ...
        T.delayedCall(.1, checkLoginStatus);
    }
}
...
function checkLoginStatus() {
    $.ajax({
        ...
        if (a[0] === "present") {
            ...
            // not logged in or resetting
            QMsg("Create an account or login to begin");
            setLoginView();
        }
    });
}
...
g.showCrystals = function() {
    ...
    $("#storeWrap, #payment-form").css('display', 'block');
    $("#card-number").focus();
...
g.storeWrap = function() {
    $("#storeWrap, #payment-form").css('display', 'none');
}
$(document).ready(function() {
    ...
    $("#payment-confirm").on('click', function(e) {
        ...
        // Validate the number:
        if (oldcard === "true") {
            D.getElementById('payment-errors').textContent = '';
            var response = {};
            response.id = "oldCard";
            ...
            if (response.error) {
                ...
                if (valid === true) {
                    $.ajax({
                        ...
                        var a = data.split("|");
                        if (a[0] === "Error") {
                            D.getElementById('payment-errors').textContent = a[1];
                        } else {
                            g.storeWrap();
                            var o = {};
                            if (amount === 1000) {
                            ...
                            var e = D.getElementById('crystalCount');
...
$(".floater").on('click', function() {
    var that = this;
    var e = D.getElementsByClassName('floater');
    for (var i = 0; i < e.length; i++) {
        e[i].className = "floater";
    }
    that.className += " purchase";
    var amount = parseInt($(".purchase").data('amount'), 10);
    var e1 = D.getElementById('crystalsExplained');
    if (amount === 1000) {
