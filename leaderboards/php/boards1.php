<?php
	...
	function leaderboardHardcore(){
		...
		$filter = $_POST['filter'];
		if($filter=="OVERALL"){
			$query = "SELECT title, name, lastName, level, exp, job, race, deaths from characters where hardcoreMode='true' ORDER BY exp DESC, row asc LIMIT 300";
		}else{ 
			$query = "SELECT title, name, lastName, level, exp, job, race, deaths from characters where job='".$filter."' and hardcoreMode='true' ORDER BY exp DESC, row asc LIMIT 100";
		}
