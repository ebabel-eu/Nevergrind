...
<html lang="en">
<head>
	<title>Nevergrind Leaderboards | Normal Mode</title>
	<meta charset="UTF-8">
	...
	<div id="leaderboards" class="strongShadow">
		<div id="leaderboardHeader">
			...
			<?php
				...
				if($_GET['board']==='exp'){
				 ...
				 echo "<a id='normalFilter' class='modeFilterActive' href='{$path}{$hardcoreURL}{$url1}{$url3}'>NORMAL</a>";
				 echo "<a id='hardcoreFilter' class='modeFilterDisabled' href='{$path}hardcore/{$url1}{$url3}'>HARDCORE</a>";
			?>
		</div>
		...
		<div id="leaderboardContainer">
			...
			<?php
				...
				if($_GET['class']==='overall'){
					$query = "select title, name, lastName, level, $dbField, job, race from characters where hardcoreMode=? order by $dbField desc, row asc limit ?";
					$stmt = $link->prepare($query);
					$stmt->bind_param('si', $hardcoreMode, $limit);
				}else{
					$query = "select title, name, lastName, level, $dbField, job, race from characters where hardcoreMode=? and job=? order by $dbField desc, row asc limit ?";
					$stmt = $link->prepare($query);
					$stmt->bind_param('ssi', $hardcoreMode, $_GET['class'], $limit);
				}
				$stmt->execute();
				$stmt->bind_result($title, $name, $lastName, $level, $score, $job, $race);
				$s = "<tr>".
				...
				$row = 1;
				while($stmt->fetch()){
					$s .="<tr><td>$row</td><td><a href='https://nevergrind.com/nevergrounds/index.php?character=".urlencode($name)."'>";
					$finalName = '';
					if($title!=''){
